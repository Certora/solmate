...
rule unexpectedBalanceChange(method f, env e) 
    filtered { f -> f.selector != safeTransferFrom(address, address, uint256, uint256, bytes).selector
                        ...
                        && f.selector != batchMint(address, uint256[], uint256[], bytes).selector  
                        && f.selector != burn(address, uint256, uint256).selector 
                        && f.selector != batchBurn(address, uint256[], uint256[]).selector } {
...
// STATUS - verified
// Chech that `batchMint()` revertes in planned scenario(s) (only if `to` is 0 or arrays have different length)
rule batchMintRevertCases(env e){
    address to; uint256[] ids; uint256[] amounts; bytes data;

    require ids.length < 1000000000;
    require amounts.length < 1000000000;

    batchMint@withrevert(e, to, ids, amounts, data);
...
// STATUS - verified
// check that batchMint updates `bootcamp participantsfrom` balance correctly
rule batchMintCorrectWork(env e){
    address to;
    uint256 id1; uint256 id2; uint256 id3; 
    ...
    batchMint(e, to, ids, amounts, data);
...
// STATUS - verified
// batchMint changes only `to` balance
rule cantBatchMintOtherBalances(env e){
    address to;
    uint256 id1; uint256 id2; uint256 id3; 
    uint256[] ids; uint256[] amounts;
    address other;
...
rule batchBurnRevertCases(env e){
    address from; uint256[] ids; uint256[] amounts;

    require ids.length < 1000000000;
    require amounts.length < 1000000000;

    batchBurn@withrevert(e, from, ids, amounts);
...
// STATUS - verified
// check that batchBurn updates `from` balance correctly
rule batchBurnCorrectWork(env e){
    address from;
    uint256 id1; uint256 id2; uint256 id3; 
    uint256 amount1; uint256 amount2; uint256 amount3;
    uint256[] ids; uint256[] amounts;
...
// STATUS - verified
// batchBurn changes only `from` balance
rule cantBatchBurnOtherBalances(env e){
    address from;
    uint256 id1; uint256 id2; uint256 id3; 
    uint256 amount1; uint256 amount2; uint256 amount3;
    uint256[] ids; uint256[] amounts;
